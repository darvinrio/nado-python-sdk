...
- **`tests/nado_client/test_market_api.py`**: Updated `test_burn_nlp` to include `productId` in the expected request structure.

### Bug Fixes
- **Multiple Field Validators**: Merged conflicting `@field_validator` decorators in `nado_protocol/engine_client/types/execute.py` for `CancelOrdersRequest`, `WithdrawCollateralRequest`, `LiquidateSubaccountRequest`, `MintNlpRequest`, `BurnNlpRequest`, and `LinkSignerRequest`. Pydantic v2 does not allow multiple validators on the same field.
- **Subaccount Serialization**: Updated `nado_protocol/utils/bytes32.py` to handle `dict` inputs in `subaccount_to_bytes32`, ensuring `sender` field is correctly serialized to bytes even when passed as a dict.
