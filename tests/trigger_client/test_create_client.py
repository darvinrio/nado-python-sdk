...
def test_create_client_signer_validation(url: str, private_keys: list[str]):
    ...
    # key.hex() doesn't include 0x prefix in newer eth-account
    assert "0x" + opts_signer_from_private_key.signer.key.hex() == private_keys[0]
    ...
    assert (
        "0x" + opts_linked_signer_from_private_key.linked_signer.key.hex() == private_keys[1]
    )
    ...
    assert "0x" + opts_signer_from_account.signer.key.hex() == private_keys[0]
    ...
    assert "0x" + opts_linked_signer_from_account.linked_signer.key.hex() == private_keys[1]
